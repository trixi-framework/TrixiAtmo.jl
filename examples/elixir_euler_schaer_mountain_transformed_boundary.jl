...
@inline function source(u, x, t, equations::CompressibleEulerEquations2D)
    return (source_terms_rayleigh_sponge(u, x, t,
                                                       equations::CompressibleEulerEquations2D) +
            source_terms_rayleigh_sponge_right(u, x, t,
                                                       equations::CompressibleEulerEquations2D) +
            source_terms_rayleigh_sponge_left(u, x, t,
                                                       equations::CompressibleEulerEquations2D) +
...
# Mountain height (positive upward)
h(x) = h_c * exp(-(x / a_c)^2) * cos(pi * x / lambda_c)^2

# Map reference parameter s ∈ [-1, 1] -> [0, 1]
s01(s) = 0.5 * (s + 1)

# x along bottom/top for ξ ∈ [-1, 1]
x_of_xi(xi) = xmin + (xmax - xmin) * s01(xi)

# Terrain heights at side walls
hL = h(xmin)
hR = h(xmax)

# faces tuple must be: (left, right, bottom, top)

# f_left: left boundary, bottom -> top
f_left(eta) = begin
    s = s01(eta)
    y = hL + (H - hL) * s
    SVector(xmin, y)
...
# f_right: right boundary, bottom -> top
f_right(eta) = begin
    s = s01(eta)
    y = hR + (H - hR) * s
    SVector(xmax, y)
...
# f_bottom: bottom boundary, left -> right
f_bottom(xi) = begin
    x = x_of_xi(xi)
    SVector(x, h(x))
end

# f_top: top boundary, left -> right
f_top(xi) = begin
    x = x_of_xi(xi)
    SVector(x, H)
end
...
mesh = StructuredMesh(cells_per_dimension, (f_left, f_right, f_bottom, f_top),
                      periodicity=false)
...
tspan = (0.0, 60 * 60*5)# * 10)  # 10h = 36000 s
