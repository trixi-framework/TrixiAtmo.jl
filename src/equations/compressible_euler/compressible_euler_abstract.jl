...
@inline function velocity(u,
                          equations::AbstractCompressibleEulerAtmo{NDIMS}) where {NDIMS}
...
@inline function prim2density_total(prim,
    ...
    r_d = 1 -
          sum(prim[SVector{NVARS - NPASSIVE - NDIMS - 2}((NDIMS + 3):(NVARS - NPASSIVE))])
...
# By convention refers to conservative variables in the form
# (rho u, rho v, [rho w, ] rho X, rho, ... ) with X being the thermodynamic quantitiy
# TODO no rearrange?
...
transform_source_terms(source_terms, equations::AbstractCompressibleEulerAtmo) = transform_source_terms_sum((source_terms,),
                                                                                                            equations)
end # @muladd
