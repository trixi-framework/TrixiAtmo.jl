...
@muladd begin
    ...
    struct CompressibleRainyEulerExplicitEquations2D{RealT <: Real} <:
           AbstractCompressibleRainyEulerEquations{2, 7}
    ...
    function CompressibleRainyEulerExplicitEquations2D(; RealT = Float64)
        ...
        return CompressibleRainyEulerExplicitEquations2D{RealT}(c_liquid_water,
                                                                c_dry_air_const_pressure,
                                                                c_dry_air_const_volume,
                                                                c_vapour_const_pressure,
                                                                c_vapour_const_volume,
                                                                R_dry_air,
                                                                R_vapour, eps,
    ...
    varnames(::typeof(cons2cons), ::CompressibleRainyEulerExplicitEquations2D) = ("rho_dry",
                                                                                  "rho_vapour",
                                                                                  "rho_cloud",
                                                                                  "rho_rain",
                                                                                  "rho_v1",
    ...
    varnames(::typeof(cons2prim), ::CompressibleRainyEulerExplicitEquations2D) = ("rho_dry",
                                                                                  "rho_vapour",
                                                                                  "rho_cloud",
                                                                                  "rho_rain",
                                                                                  "v1",
    ...
    varnames(::typeof(cons2eq_pot_temp), ::CompressibleRainyEulerExplicitEquations2D) = ("rho_dry",
                                                                                         "rho_vapour",
                                                                                         "rho_cloud",
                                                                                         "rho_rain",
                                                                                         "v1",
    ...
    @inline function velocities(u, rho_inv,
                                equations::CompressibleRainyEulerExplicitEquations2D)
    ...
    @inline function get_temperature(u,
                                     equations::CompressibleRainyEulerExplicitEquations2D)
        # constants
        c_l = equations.c_liquid_water
        c_vd = equations.c_dry_air_const_volume
        ...
        return (energy - L_ref * rho_vapour - 0.5 * rho * (v1^2 + v2^2)) /
               (c_vd * rho_dry + c_vv * rho_vapour + c_l * (rho_cloud + rho_rain))
    end
    ...
    @inline function terminal_velocity_rain(rho_moist, rho_rain,
        ...
        if (rho_rain > 0.0)
            v_terminal_rain = v_0 * 1.9386213994279082 *
                              (rho_rain / (pi * (rho_moist + rho_rain) * N_0))^(0.125)
        else
            v_terminal_rain = 0.0
        end
    ...
    # adapted from compressible_moist_euler_2d.jl
    @inline function moist_air_phase_change(u,
                                            equations::CompressibleRainyEulerExplicitEquations2D)
        # constants
        R_v = equations.R_vapour
    ...
    @inline function flux(u, orientation::Integer,
        ...
        if (orientation == 1)
            ...
            # "energy"
            f7 = (energy + p) * v2 -
                 (c_l * temperature + 0.5 * (v1^2 + v2^2)) * rho_rain * v_r
        end
    ...
    @inline function flux(u, normal_direction::AbstractVector,
        ...
        # "energy"
        f7 = (energy + p) * v_normal -
             (c_l * temperature + 0.5 * (v1^2 + v2^2)) * rho_rain * v_r_normal
    ...
    @inline function source_terms_rainy(u, x, t,
        ...
        S_evaporation = (3.86e-3 - 9.41e-5 * (temperature - ref_temp)) *
                        (1 + 9.1 * rho_rain^(0.1875))
        ...
        return SVector(0.0, Q_ph + S_evaporation, -Q_ph - S_auto_conversion - S_accretion,
                       S_rain, 0.0,
                       -rho * g, -rho_v2 * g)
    end

    # no Coriolis term
    @inline function source_terms_moist(u, x, t,
                                        equations::CompressibleRainyEulerExplicitEquations2D)
        # constants
        g = equations.gravity
    ...
    @inline function boundary_condition_slip_wall(u_inner, normal_direction::AbstractVector,
        ...
        # densities
        rho_dry_local, rho_vapour_local, rho_cloud_local, rho_rain_local, rho_local, rho_inv_local = densities(u_local,
                                                                                                               equations)
    ...
    @inline function flux_LMARS(u_ll, u_rr, normal_direction::AbstractVector,
        ...
        # densities
        rho_dry_ll, rho_vapour_ll, rho_cloud_ll, rho_rain_ll, rho_ll, rho_inv_ll = densities(u_ll,
                                                                                             equations)
        rho_dry_rr, rho_vapour_rr, rho_cloud_rr, rho_rain_rr, rho_rr, rho_inv_rr = densities(u_rr,
                                                                                             equations)
    ...
    @inline function flux_chandrashekar(u_ll, u_rr, normal_direction::AbstractVector,
        ...
        f_rhov1 = (f_dry + f_vapour + f_cloud + f_rain) * v1_avg +
                  normal_direction[1] * p_int
        f_rhov2 = (f_dry + f_vapour + f_cloud + f_rain) * v2_avg +
                  normal_direction[2] * p_int
