...
function initial_condition_gaussian_bell(x, t, equations)
    ...
    (λ_0, θ_0) = (convert(RealT, 3π / 2), convert(RealT, 0.0f0))
...
semis = [SemidiscretizationHyperbolic(mesh, equations[i],
                                      initial_condition_gaussian_bell, solver,
                                      source_terms = nothing,
                                      boundary_conditions = connectivity[i])
...
sol = solve(ode, CarpenterKennedy2N54(), dt = 1.0, save_everystep = false,
            callback = callbacks)
