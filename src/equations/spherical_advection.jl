...
struct SphericalLinearAdvectionEquation2D{Manifold <:
                                          AbstractManifold{2, 3}} <:
       Trixi.AbstractEquations{2, 4}
    manifold::Manifold
end

function Trixi.varnames(::typeof(cons2cons), ::SphericalLinearAdvectionEquation2D)
    ("scaled height",  # height scaled by area element
     "area element",  # area element
     "contravariant vel. 1", "contravariant vel. 2")  # contravariant velocity components
end  # contravariant velocity components

function Trixi.varnames(::typeof(cons2prim), ::SphericalLinearAdvectionEquation2D)
    ("height",  # height scaled by area element
     "area element",  # area element
     "contravariant vel. 1", "contravariant vel. 2")  # contravariant velocity components
end  # contravariant velocity components
...
@inline velocity_contravariant(u, ::SphericalLinearAdvectionEquation2D) = SVector(u[3],
                                                                                  u[4])

""" 
    Convert conservative to primitive variables
"""
@inline Trixi.cons2prim(u, ::SphericalLinearAdvectionEquation2D) = SVector(u[1] / u[2],
                                                                           u[2], u[3], u[4])

""" 
    Convert primitive to conservative variables
"""
@inline Trixi.prim2cons(q, ::SphericalLinearAdvectionEquation2D) = SVector(u[1] * u[2],
                                                                           u[2], u[3], u[4])
