...
equations_uni = [
    CompressibleEulerAtmo(; n_dims = 1,
                          parameters = parameters,
                          thermodynamic_state = td_single,
                          thermodynamic_equation = td_potT),
    CompressibleEulerAtmo(; n_dims = 2,
                          parameters = parameters,
                          thermodynamic_state = td_single,
                          thermodynamic_equation = td_potT),
    CompressibleEulerAtmo(; n_dims = 3,
                          parameters = parameters,
                          thermodynamic_state = td_single,
                          thermodynamic_equation = td_potT)]
...
equations_orig = [CompressibleEulerPotentialTemperatureEquations1D(c_p = c_p, c_v = c_v),
    CompressibleEulerPotentialTemperatureEquations2D(c_p = c_p, c_v = c_v),
    CompressibleEulerPotentialTemperatureEquations3D(c_p = c_p, c_v = c_v)]
...
for dim in 2:3
    ...
    for n in 1:(dim + 2)
        ...
        for (iflux, _flux) in enumerate(fluxes)
            ...
            @test flux_rearrange ≈
                  _flux(u_uni_ll, u_uni_rr, normal_direction, equations_uni[dim])
