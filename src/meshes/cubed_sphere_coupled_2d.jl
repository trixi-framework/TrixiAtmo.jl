...
struct CubedSphereFace2D{face_number, RealT <: Real} <: AbstractManifold{2, 3}
    ...
    function CubedSphereFace2D{face_number}(a::RealT) where {face_number, RealT <: Real}
...
function cubed_sphere_connectivity(coupling_function)
    return [
            (x_neg = BoundaryConditionCoupled(4, (:end, :i_forward), Float64,
                                              coupling_function),
             x_pos = BoundaryConditionCoupled(2, (:begin, :i_forward), Float64,
                                              coupling_function),
             y_neg = BoundaryConditionCoupled(6, (:i_forward, :end), Float64,
                                              coupling_function),
             y_pos = BoundaryConditionCoupled(5, (:i_forward, :begin), Float64,
                                              coupling_function)),
            (x_neg = BoundaryConditionCoupled(1, (:end, :i_forward), Float64,
                                              coupling_function),
             x_pos = BoundaryConditionCoupled(3, (:begin, :i_forward), Float64,
                                              coupling_function),
             y_neg = BoundaryConditionCoupled(6, (:end, :i_backward), Float64,
                                              coupling_function),
             y_pos = BoundaryConditionCoupled(5, (:end, :i_forward), Float64,
                                              coupling_function)),
            (x_neg = BoundaryConditionCoupled(2, (:end, :i_forward), Float64,
                                              coupling_function),
             x_pos = BoundaryConditionCoupled(4, (:begin, :i_forward), Float64,
                                              coupling_function),
             y_neg = BoundaryConditionCoupled(6, (:i_backward, :begin), Float64,
                                              coupling_function),
             y_pos = BoundaryConditionCoupled(5, (:i_backward, :end), Float64,
                                              coupling_function)),
            (x_neg = BoundaryConditionCoupled(3, (:end, :i_forward), Float64,
                                              coupling_function),
             x_pos = BoundaryConditionCoupled(1, (:begin, :i_forward), Float64,
                                              coupling_function),
             y_neg = BoundaryConditionCoupled(6, (:begin, :i_forward), Float64,
                                              coupling_function),
             y_pos = BoundaryConditionCoupled(5, (:begin, :i_backward), Float64,
                                              coupling_function)),
            (x_neg = BoundaryConditionCoupled(4, (:i_backward, :end), Float64,
                                              coupling_function),
             x_pos = BoundaryConditionCoupled(2, (:i_forward, :end), Float64,
                                              coupling_function),
             y_neg = BoundaryConditionCoupled(1, (:i_forward, :end), Float64,
                                              coupling_function),
             y_pos = BoundaryConditionCoupled(3, (:i_backward, :end), Float64,
                                              coupling_function)),
            (x_neg = BoundaryConditionCoupled(4, (:i_forward, :begin), Float64,
                                              coupling_function),
             x_pos = BoundaryConditionCoupled(2, (:i_backward, :begin), Float64,
                                              coupling_function),
             y_neg = BoundaryConditionCoupled(3, (:i_backward, :begin), Float64,
                                              coupling_function),
             y_pos = BoundaryConditionCoupled(1, (:i_forward, :begin), Float64,
                                              coupling_function))]
end
...
@inline function basis_contravariant(x, manifold::CubedSphereFace2D{1})
    ...
    return secθ_loc * secλ_loc / manifold.a *
           SMatrix{2, 2}(cos²x1 * secλ_loc, cos²x2 * tan(θ_loc) * tan(λ_loc),
                         0, cos²x2 * secθ_loc)
end
...
@inline function basis_contravariant(x, manifold::CubedSphereFace2D{5})
    ...
    return 1 / (manifold.a * sinθ^2) *
           SMatrix{2, 2}(cos²x1 * sinθ * cosλ, cos²x2 * sinθ * sinλ, -cos²x1 * sinλ,
                         cos²x2 * cosλ)
end
