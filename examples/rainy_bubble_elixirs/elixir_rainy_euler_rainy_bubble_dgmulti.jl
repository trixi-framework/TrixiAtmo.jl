...
function generate_hydrostatic_residual(pressure_lower, humidity_rel0, z, dz, equations::CompressibleRainyEulerEquations2D)
    ...
    function hydrostatic_residual!(residual, guess)
        ...
        residual[3] = theta_d(z, equations) - temperature * (ref_pressure / pressure)^(R_d / c_pd)
...
function generate_perturbation_residual(pressure_hydrostatic, H_init, z, equations::CompressibleRainyEulerEquations2D)
    ...
    function perturbation_residual!(residual, guess)
        ...
        # hydrostatic dry potential temperature residual
        residual[2] = theta_d(z, equations) - temperature * (ref_pressure / pressure_hydrostatic)^(R_d / c_pd)
...
function AtmosphereLayers(equations::CompressibleRainyEulerEquations2D; total_height = coordinates_max[2] + 1.0, precision = 1.0, RealT = Float64)
    ...
    dz = 0.01
    z  = 0.01
    residual_function!  = generate_hydrostatic_residual(surface_pressure, humidity_rel0, z, dz, equations)
...
function initial_condition_rainy_bubble(x, t, equations::CompressibleRainyEulerEquations2D; atmosphere_layers = layers)
    # equations constants
    ...
    pressure_hydrostatic       = (pressure_hydrostatic_upper * (z - z_lower) + pressure_hydrostatic_lower * (z_upper - z)) / dz
...
initial_condition = initial_condition_rainy_bubble
