...
@muladd begin
function nonlinear_solve_dg_2d!(u, nonlin_fct::NonlinearFunction{false},
    ...
    for element in eachelement(dg, cache)
        for j in eachnode(dg), i in eachnode(dg)
            ...
            prob = NonlinearProblem{false}(nonlin_fct, initial_guess,
                                            (u_node, equations))
            # TODO: abs and rel?
            res = solve(prob, SimpleNewtonRaphson(); abstol = tolerance,
                        reltol = tolerance)
