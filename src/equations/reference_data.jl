...
"""
@inline function initial_condition_gaussian(x, t, equations::AbstractEquations)
    RealT = eltype(x)
...
function initial_condition_geostrophic_balance(x, t,
    ...
    # convert primitive variables from spherical coordinates to the chosen coordinate system
    return spherical2global(SVector(h, vlon, vlat, zero(eltype(x))), x, equations)
end
end # muladd
