...
"""
    flux_kennedy_gruber_souza_etal_turbo(u_ll, u_rr, orientation_or_normal_direction, equations)

Equivalent to the combined [`flux_kennedy_gruber`](@ref) ['flux_nonconservative_souza_etal'] except that it may use specialized
...
@inline function Trixi.flux_differencing_kernel!(_du::PtrArray, u_cons::PtrArray,
                                                 ...
                                                 volume_flux::typeof(flux_kennedy_gruber_souza_etal_turbo),
    ...
    for i in eachnode(dg), ii in (i + 1):nnodes(dg)
        @turbo for jk in Base.OneTo(nnodes(dg)^2)
            ...
            e_ll = u_prim_permuted[jk, i, 5]
            p_ll = u_prim_permuted[jk, i, 6]
            ...
            e_rr = u_prim_permuted[jk, ii, 5]
            p_rr = u_prim_permuted[jk, ii, 6]
            ...
            gravity = rho_avg * phi_jump
            g2 = 0.5f0 * normal_direction_1 * gravity
            ...
            du_permuted[jk, i, 3] += factor_i * (f3 + g3)
            du_permuted[jk, i, 4] += factor_i * (f4 + g4)
            du_permuted[jk, i, 5] += factor_i * (f5 + g5)

            factor_ii = alpha * derivative_split[ii, i]
            du_permuted[jk, ii, 1] += factor_ii * f1
            du_permuted[jk, ii, 2] += factor_ii * (f2 - g2)
            du_permuted[jk, ii, 3] += factor_ii * (f3 - g3)
            du_permuted[jk, ii, 4] += factor_ii * (f4 - g4)
            du_permuted[jk, ii, 5] += factor_ii * (f5 - g5)
        end
    end
    ...
    for j in eachnode(dg), jj in (j + 1):nnodes(dg)
        @turbo for k in eachnode(dg), i in eachnode(dg)
            ...
            e_rr = u_prim[i, jj, k, 5]
            p_rr = u_prim[i, jj, k, 6]
            ...
            g3 = 0.5f0 * normal_direction_2 * gravity
            g4 = 0.5f0 * normal_direction_3 * gravity
            g5 = rho_avg * phi_jump * v_dot_n_avg
            # Add scaled fluxes to RHS
            factor_j = alpha * derivative_split[j, jj]
            du[i, j, k, 1] += factor_j * f1
            du[i, j, k, 2] += factor_j * (f2 + g2)
            du[i, j, k, 3] += factor_j * (f3 + g3)
            du[i, j, k, 4] += factor_j * (f4 + g4)
            du[i, j, k, 5] += factor_j * (f5 + g5)

            factor_jj = alpha * derivative_split[jj, j]
            du[i, jj, k, 1] += factor_jj * f1
            du[i, jj, k, 2] += factor_jj * (f2 - g2)
            du[i, jj, k, 3] += factor_jj * (f3 - g3)
            du[i, jj, k, 4] += factor_jj * (f4 - g4)
            du[i, jj, k, 5] += factor_jj * (f5 - g5)
        end
    end
    ...
    GC.@preserve u_prim begin
        ...
        for k in eachnode(dg), kk in (k + 1):nnodes(dg)
            @turbo for ij in Base.OneTo(nnodes(dg)^2)
                ...
                e_ll = u_prim_reshaped[ij, k, 5]
                p_ll = u_prim_reshaped[ij, k, 6]
                ...
                e_rr = u_prim_reshaped[ij, kk, 5]
                p_rr = u_prim_reshaped[ij, kk, 6]
                ...
                du_reshaped[ij, k, 2] += factor_k * (f2 + g2)
                du_reshaped[ij, k, 3] += factor_k * (f3 + g3)
                du_reshaped[ij, k, 4] += factor_k * (f4 + g4)
                du_reshaped[ij, k, 5] += factor_k * (f5 + g5)

                factor_kk = alpha * derivative_split[kk, k]
                du_reshaped[ij, kk, 1] += factor_kk * f1
                du_reshaped[ij, kk, 2] += factor_kk * (f2 - g2)
                du_reshaped[ij, kk, 3] += factor_kk * (f3 - g3)
                du_reshaped[ij, kk, 4] += factor_kk * (f4 - g4)
                du_reshaped[ij, kk, 5] += factor_kk * (f5 - g5)
            end
        end
