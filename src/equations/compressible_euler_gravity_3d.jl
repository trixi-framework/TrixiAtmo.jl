...
function Trixi.varnames(::typeof(cons2prim), ::CompressibleEulerEquationsWithGravity3D)
    ("rho", "v1", "v2", "v3", "p", "phi")
end
...
@inline function Trixi.flux_chandrashekar(u_ll, u_rr, normal_direction::AbstractVector,
                                          equations::CompressibleEulerEquationsWithGravity3D)
    # Unpack left and right state
    rho_ll, v1_ll, v2_ll, v3_ll, p_ll, _ = cons2prim(u_ll, equations)
    rho_rr, v1_rr, v2_rr, v3_rr, p_rr, _ = cons2prim(u_rr, equations)
...
@inline function Trixi.rotate_to_x(u, normal_vector, tangent1, tangent2,
                                   equations::CompressibleEulerEquationsWithGravity3D)
...
@inline function Trixi.rotate_from_x(u, normal_vector, tangent1, tangent2,
                                     equations::CompressibleEulerEquationsWithGravity3D)
    # Multiply with [ 1        0          0        0      0;
    #                 0        |          |        |      0;
...
@inline function Trixi.velocity(u, orientation::Int,
                                equations::CompressibleEulerEquationsWithGravity3D)
    rho = u[1]
...
@inline function Trixi.pressure(u, equations::CompressibleEulerEquationsWithGravity3D)
...
@inline function Trixi.entropy_thermodynamic(u,
                                             equations::CompressibleEulerEquationsWithGravity3D)
    rho, _ = u
...
@inline function Trixi.entropy(cons, equations::CompressibleEulerEquationsWithGravity3D)
    entropy_math(cons, equations)
end

# Calculate total energy for a conservative state `cons`
@inline Trixi.energy_total(cons, ::CompressibleEulerEquationsWithGravity3D) = cons[5]
