...
function mapping(xi_, eta_)
    ...
    dtopo = (2*h_c*exp(-xi^2/a_c^2)*cos(pi*xi/lambda_c)*(pi*a_c^2*sin(pi*xi/lambda_c)+lambda_c*xi*cos(pi*xi/lambda_c)))/(a_c^2*lambda_c)
    theta = acos(1/sqrt((dtopo)^2+1))
    n = 5

    b = ((H - eta) / (H - topo))^n

    x = xi * cos(theta * b) + eta * sin(theta * b)
    y = -xi* sin(theta * b) + eta * cos(theta * b) 
    return SVector(x, y)
end
...
tspan = (0.0, 60)#60 * 60 * 10)  # 10h = 36000 s
