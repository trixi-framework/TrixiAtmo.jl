...
@trixi_testset "elixir_euler_potential_temperature_tec" begin
    @test_trixi_include(joinpath(EXAMPLES_DIR,
                        ...
                        tspan=(0.0, 0.4), surface_flux=flux_tec, volume_flux=flux_tec, atol=8.0e-11)
    # Ensure that we do not have excessive memory allocations
    # (e.g., from type instabilities)
    @test_allocations(TrixiAtmo.Trixi.rhs!, semi, sol, 100)
end
...
@trixi_testset "elixir_euler_potential_temperature_etec" begin
    @test_trixi_include(joinpath(EXAMPLES_DIR,
                        ...
                        tspan=(0.0, 0.4), surface_flux=flux_etec, volume_flux=flux_etec, atol=8.0e-11)
    # Ensure that we do not have excessive memory allocations
    # (e.g., from type instabilities)
    @test_allocations(TrixiAtmo.Trixi.rhs!, semi, sol, 100)
end
