...
function initial_condition_dry_air_warm_bubble_generator(parameters::Parameters{RealType};
    ...
    function initial_condition(x, t, equations)
        ...
        if r <= perturbation_radius
            potential_temperature_perturbation = 2 *
                                                 cospi(0.5f0 * r / perturbation_radius)^2
        end
...
function moist_state(y, dz, y0, r_t0, theta_e0,
                     parameters::Parameters{RealType}) where {RealType}
...
function initial_condition_bryan_fritsch_bubble_generator(parameters::Parameters{RealType};
                                                          moist = true,
                                                          perturbation_center_x = RealType(10000),
                                                          perturbation_center_z = RealType(2000),
    ...
    function perturb_moist_profile!(x, rho, rho_theta, rho_qv, rho_ql)
        ...
        if (r < perturbation_radius && Δθ > 0)
            ...
            θ_dens_new = θ_dens *
                         (1 + Δθ * cospi(0.5f0 * r / perturbation_radius)^2 / 300)
            ...
            if rt > 0
                while true
                    ...
                    pvs = convert(RealType, 611.2) *
                          exp(convert(RealType, 17.62) * T_C /
                              (convert(RealType, 243.12) + T_C))
            ...
            rho_e = (c_vd * rho_d + c_vv * rho_qv + c_pl * rho_ql) * T_loc +
                    L_00 * rho_qv
        end
    ...
    function initial_condition(x, t, equations)
        ...
        rho_theta = rho *
                    ...
                    dz
        rho_qv = rho * (rho_qv_r / rho_r * (z - z_l) + rho_qv_l / rho_l * (z_r - z)) /
                 dz
        rho_ql = rho * (rho_ql_r / rho_r * (z - z_l) + rho_ql_l / rho_l * (z_r - z)) /
                 dz

        rho, rho_e, rho_qv, rho_ql, T_loc, p_loc = perturb_moist_profile!(x, rho,
                                                                          rho_theta,
                                                                          rho_qv,
                                                                          rho_ql)
        v1 = 0
